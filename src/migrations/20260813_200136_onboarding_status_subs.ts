...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  await db.run(sql`CREATE TABLE \`onboarding_steps\` (
  	...
  	\`note\` text,
  	FOREIGN KEY (\`_parent_id\`) REFERENCES \`onboarding\`(\`id\`) ON UPDATE no action ON DELETE cascade
  );
  `)
  await db.run(sql`CREATE INDEX \`onboarding_steps_order_idx\` ON \`onboarding_steps\` (\`_order\`);`)
  await db.run(sql`CREATE INDEX \`onboarding_steps_parent_id_idx\` ON \`onboarding_steps\` (\`_parent_id\`);`)
  ...
  `)
  await db.run(sql`CREATE INDEX \`onboarding_customer_idx\` ON \`onboarding\` (\`customer_id\`);`)
  await db.run(sql`CREATE INDEX \`onboarding_updated_at_idx\` ON \`onboarding\` (\`updated_at\`);`)
  await db.run(sql`CREATE INDEX \`onboarding_created_at_idx\` ON \`onboarding\` (\`created_at\`);`)
  await db.run(sql`CREATE TABLE \`status_subscribers\` (
  	\`id\` integer PRIMARY KEY NOT NULL,
  	\`email\` text NOT NULL,
  ...
  `)
  await db.run(sql`CREATE UNIQUE INDEX \`status_subscribers_email_idx\` ON \`status_subscribers\` (\`email\`);`)
  await db.run(sql`CREATE INDEX \`status_subscribers_updated_at_idx\` ON \`status_subscribers\` (\`updated_at\`);`)
  await db.run(sql`CREATE INDEX \`status_subscribers_created_at_idx\` ON \`status_subscribers\` (\`created_at\`);`)
  await db.run(sql`ALTER TABLE \`payload_locked_documents_rels\` ADD \`onboarding_id\` integer REFERENCES onboarding(id);`)
  await db.run(sql`ALTER TABLE \`payload_locked_documents_rels\` ADD \`status_subscribers_id\` integer REFERENCES status_subscribers(id);`)
...
export async function down({ db, payload, req }: MigrateDownArgs): Promise<void> {
  await db.run(sql`DROP TABLE \`onboarding_steps\`;`)
  await db.run(sql`DROP TABLE \`onboarding\`;`)
  await db.run(sql`DROP TABLE \`status_subscribers\`;`)
