...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  ...
  await db.run(sql`CREATE TABLE \`services\` (
  	...
  	\`icon\` text,
  	\`accent\` text DEFAULT 'from-cyan-glow to-violet-glow',
  ...
  await db.run(sql`CREATE TABLE \`products_bullets\` (
  	\`_order\` integer NOT NULL,
  	\`_parent_id\` integer NOT NULL,
  	\`id\` text PRIMARY KEY NOT NULL,
  ...
  `)
  await db.run(sql`CREATE INDEX \`products_bullets_order_idx\` ON \`products_bullets\` (\`_order\`);`)
  await db.run(sql`CREATE INDEX \`products_bullets_parent_id_idx\` ON \`products_bullets\` (\`_parent_id\`);`)
...
export async function down({ db, payload, req }: MigrateDownArgs): Promise<void> {
  ...
  await db.run(sql`DROP TABLE \`products_bullets\`;`)
