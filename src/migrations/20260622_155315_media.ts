...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  ...
  await db.run(sql`ALTER TABLE \`partners\` ADD \`logo_media_id\` integer REFERENCES media(id);`)
  await db.run(sql`CREATE INDEX \`partners_logo_media_idx\` ON \`partners\` (\`logo_media_id\`);`)
  await db.run(sql`ALTER TABLE \`projects\` ADD \`before_media_id\` integer REFERENCES media(id);`)
  await db.run(sql`ALTER TABLE \`projects\` ADD \`after_media_id\` integer REFERENCES media(id);`)
  await db.run(sql`CREATE INDEX \`projects_before_media_idx\` ON \`projects\` (\`before_media_id\`);`)
  await db.run(sql`CREATE INDEX \`projects_after_media_idx\` ON \`projects\` (\`after_media_id\`);`)
...
export async function down({ db, payload, req }: MigrateDownArgs): Promise<void> {
  ...
  `)
  await db.run(sql`INSERT INTO \`__new_posts\`("id", "title", "slug", "excerpt", "category", "author", "status", "date", "read_mins", "body", "updated_at", "created_at") SELECT "id", "title", "slug", "excerpt", "category", "author", "status", "date", "read_mins", "body", "updated_at", "created_at" FROM \`posts\`;`)
  ...
  `)
  await db.run(sql`INSERT INTO \`__new_partners\`("id", "name", "logo", "order", "updated_at", "created_at") SELECT "id", "name", "logo", "order", "updated_at", "created_at" FROM \`partners\`;`)
  ...
  `)
  await db.run(sql`INSERT INTO \`__new_projects\`("id", "title", "slug", "industry", "summary", "before_image", "after_image", "url", "order", "updated_at", "created_at") SELECT "id", "title", "slug", "industry", "summary", "before_image", "after_image", "url", "order", "updated_at", "created_at" FROM \`projects\`;`)
