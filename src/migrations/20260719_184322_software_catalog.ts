...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  await db.run(sql`CREATE TABLE \`software_plans\` (
  	\`_order\` integer NOT NULL,
  	\`_parent_id\` integer NOT NULL,
  	\`id\` text PRIMARY KEY NOT NULL,
  	\`name\` text NOT NULL,
  	\`price\` numeric,
  	\`unit\` text DEFAULT 'per user / month',
  	\`feature\` text,
  	FOREIGN KEY (\`_parent_id\`) REFERENCES \`software\`(\`id\`) ON UPDATE no action ON DELETE cascade
  );
  `)
  await db.run(sql`CREATE INDEX \`software_plans_order_idx\` ON \`software_plans\` (\`_order\`);`)
  await db.run(sql`CREATE INDEX \`software_plans_parent_id_idx\` ON \`software_plans\` (\`_parent_id\`);`)
  await db.run(sql`CREATE TABLE \`software_addons\` (
  	\`_order\` integer NOT NULL,
  	\`_parent_id\` integer NOT NULL,
  	\`id\` text PRIMARY KEY NOT NULL,
  	\`name\` text NOT NULL,
  ...
  `)
  await db.run(sql`CREATE INDEX \`software_addons_order_idx\` ON \`software_addons\` (\`_order\`);`)
  await db.run(sql`CREATE INDEX \`software_addons_parent_id_idx\` ON \`software_addons\` (\`_parent_id\`);`)
...
export async function down({ db, payload, req }: MigrateDownArgs): Promise<void> {
  await db.run(sql`DROP TABLE \`software_plans\`;`)
  await db.run(sql`DROP TABLE \`software_addons\`;`)
