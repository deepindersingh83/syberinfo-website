...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  await db.run(sql`CREATE TABLE \`assets\` (
  	...
  	\`auto_remind\` integer DEFAULT true,
  ...
  `)
  await db.run(sql`CREATE INDEX \`assets_customer_idx\` ON \`assets\` (\`customer_id\`);`)
  ...
  await db.run(sql`ALTER TABLE \`payload_locked_documents_rels\` ADD \`assets_id\` integer REFERENCES assets(id);`)
